...
export function LeftCard({ campaign, progress }: SideCardProps) {
  return (
    <div className="w-full h-full bg-white rounded-[2rem] shadow_-soft-md border border-slate-100 overflow-hidden group hover:border-orange-200 transition-all duration-500 ease-out flex flex-col hover:scale-[1.03]">
        ...
        <div className="space-y-3 pt-4">
          ...
          <div className="flex justify-between items-center">
            ...
            <Link href={`/campaigns/${campaign.id}`}>
              <Button
                size="sm"
                className="h-10 rounded-xl bg-slate-900 text-white shadow_-md hover:bg-primary hover:shadow_-glow transition-all px-5 flex items-center gap-1.5 group/btn"
              >
                Donate
