...
export function CenterCard({ campaign, progress }: CenterCardProps) {
  return (
    <div className="w-full h-full relative z-20 shadow_-2xl rounded-[2rem] border border-slate-100 bg-white hover:border-orange-200 transition-all duration-300 flex flex-col md:flex-row overflow-hidden group">
      
      {/* Trending Badge */}
      <div className="absolute top-4 left-4 z-30 bg-gradient-tush text-white px-4 py-1 rounded-full text-xs font-bold shadow_-soft-xl animate-pulse">
        🔥 Top Trending
      </div>
      ...
      <div className="flex flex-col justify-between p-8 md:w-7/12 h-full bg-white">
        ...
        <div className="space-y-6">
          ...
          <div className="w-full bg-slate-100 rounded-full h-3 overflow-hidden">
            <div
              className="bg-gradient-tush h-full rounded-full shadow_-[0_0_15px_rgba(255,107,74,0.4)]"
          ...
          <div className="flex justify-between items-end">
            ...
            <Link href={`/campaigns/${campaign.id}`}>
              <Button className="h-12 rounded-xl px-8 bg-slate-900 text-white shadow_-lg hover:bg-primary hover:shadow_-glow transition-all duration-300 flex items-center gap-2 group/btn">
                Donate Now
