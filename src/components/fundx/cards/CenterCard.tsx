...
export function CenterCard({ campaign, progress }: CenterCardProps) {
  return (
    <div className="w-full h-full relative z-20 shadow-2xl rounded-[2rem] border border-slate-100 bg-white hover:border-orange-200 transition-all duration_-300 flex flex-col md:flex-row overflow-hidden group">
        ...
        <div className="space-y-6">
          ...
          <div className="flex justify-between items-end">
            ...
            <Link href={`/campaigns/${campaign.id}`}>
              <Button className="h-12 rounded-xl px-8 bg-slate-900 text-white shadow-lg hover:bg-primary hover:shadow-glow transition-all duration_-300 flex items-center gap-2 group/btn">
                Donate Now
                <Heart className="w-4 h-4 transition-transform duration_-300 group-hover/btn:scale-125 group-hover/btn:fill-white" />
              </Button>
            </Link>
          </div>
        </div>
