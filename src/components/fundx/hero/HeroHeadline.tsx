...
export function HeroHeadline({
  displayStacks, glitching_, glitchOffset, glitchOpacity_, glitchSkew, isStacksMode, scrambledText, onToggle,
}: {
  displayStacks: boolean; glitching_: boolean; glitchOffset: { x: number; y: number }; glitchOpacity_: number; glitchSkew: number; isStacksMode: boolean; scrambledText: string; onToggle: () => void
}) {
  return (
    <h1 className="text-6xl md:text-7xl lg:text-8xl font-bold tracking-tighter text-slate-900 leading-[1.1] mb-8">
      ...
      <span className="inline-flex items-center flex-wrap justify-center gap-x-4">
        on the
        <ChainToggleIcon displayStacks={displayStacks} glitching_={glitching_} glitchOffset={glitchOffset} glitchOpacity_={glitchOpacity_} glitchSkew={glitchSkew} isStacksMode={isStacksMode} />
