...
function HeroLogoParallax() {
  ...
  const mouseOffset_ = useRef({ x: 0, y: 0 })
  ...
  const startTimeRef = useRef<number>(0)
        ...
        currentMouse.current.x += (mouseOffset_.current.x - currentMouse.current.x) * 0.12
        currentMouse.current.y += (mouseOffset_.current.y - currentMouse.current.y) * 0.12
