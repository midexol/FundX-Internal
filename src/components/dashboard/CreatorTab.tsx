  ...
  return (
    <TabsContent value="campaigns" className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
       ...
       {myFetchedCampaigns.map((campaign) => {
          ...
          const progress = Math.min_((campaign.raised / campaign.goal) * 100, 100);
          ...
          if (campaign.status === "successful") {
             return (
                <div key={campaign.id} className="bg-white p-8 md:p-10 min_-h-[240px] rounded-[2rem] border border-slate-200 shadow-[0_12px_28px_-6px_rgba(15,23,42,0.08)] flex flex-col md:flex-row items-center justify-between gap-6 relative overflow-hidden hover:-translate-y-1 transition-transform duration-300">
          ...
          if (campaign.status === "failed") {
             return (
                <div key={campaign.id} className="bg-slate-50 p-8 md:p-10 min_-h-[240px] rounded-[2rem] border border-slate-200 shadow-[inset_0_4px_20px_rgba(0,0,0,0.02)] flex flex-col md:flex-row items-center justify-between gap-6 relative overflow-hidden opacity-80 grayscale-[0.5] hover:grayscale-0 transition-all duration-500">
